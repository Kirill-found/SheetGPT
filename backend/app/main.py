"""
SheetGPT API Production v7.2.0 - Function Calling with Smart Matching
95%+ accuracy через проверенные функции + умный поиск колонок
Автоматическое преобразование строковых чисел
Railway deployment: 2025-11-17 - FUNCTION CALLING v7.2.0
"""
...
# Create FastAPI app with VERSION 7.2.0 - Smart column matching + string number parsing
app = FastAPI(
    title="SheetGPT API",
    version="7.2.0",  # v7.2.0: Smart column matching + auto string number parsing
...
async def startup_event():
    ...
    logger.info("="*60)
    logger.info("SheetGPT API v7.2.0 STARTING - Function Calling with Smart Column Matching")
...
async def process_formula(request: FormulaRequest):
    """
    Main endpoint v7.2.0 - Function Calling with Smart Column Matching
    ...
    try:
        ...
        logger.info("="*60)
        logger.info(f"[REQUEST v7.2.0] Query: {request.query}")
        logger.info(f"[DATA] Shape: {len(request.sheet_data)} rows x {len(request.column_names)} columns")

        result = None

        # Try AIFunctionCaller first (v7.2.0 with smart matching)
        try:
            from app.services.ai_function_caller import AIFunctionCaller
            import pandas as pd

            logger.info("[ENGINE v7.2.0] Using AI Function Caller with Smart Column Matching")
