...
4. ОБЯЗАТЕЛЬНО создай переменную `explanation` со СТРУКТУРИРОВАННЫМ ответом
...
ШАБЛОН explanation для разных типов вопросов:

1. Для "кто лучший/худший" (сравнение):
```
**Ответ: [Имя]**
...
💡 Вывод: [краткое объяснение почему лидер лучше]
```

2. Для "почему?" (объяснение):
```
**[Имя] лидирует** потому что:

📈 Ключевые факты:
• Общая сумма: [X] руб.
• Разница с [второй]: [Y] руб. ([Z]% больше)
• Количество сделок: [N]

💡 [Дополнительный вывод если есть]
```

3. Для "сколько/какая сумма" (число):
```
**[Число/Сумма]**
