...
class SimpleGPTProcessor:
    ...
    FULL_DATA_KEYWORDS = [
        ...
        'найди', 'найти', 'поиск', 'где', 'который', 'какой', 'какая', 'какие',
        'покажи строки', 'покажи записи', 'выведи строки',
        # Выделение
        'выдели', 'выделить', 'подсвети', 'подсветить', 'highlight', 'mark',
        # Анализ каждого элемента
        'каждый', 'каждого', 'каждая', 'каждую', 'по каждому', 'для каждого',
        # Проценты и доли (нужны все данные для точного расчёта)
        '% от', 'процент от', 'доля от', 'долю', 'распределение',
        # Сравнение
        'сравни', 'сравнить', 'разница между', 'отличия',
        # Фильтрация с выводом
        'отфильтруй', 'фильтр', 'оставь только', 'убери', 'удали строки',
        # Дубликаты
        'дубликат', 'дубли', 'повтор', 'уникальн',
        # Проверка всех данных
        'проверь', 'проверить', 'валидация', 'ошибки в данных', 'аномалии',
        # Группировка с деталями
        'сгруппируй', 'группировка', 'разбей по',
    ]

    # Ключевые слова для SAMPLE режима (формулы, агрегации)
    SAMPLE_KEYWORDS = [
        'сумма', 'сумму', 'суммы', 'sum', 'итого', 'total',
        'среднее', 'среднюю', 'average', 'avg', 'mean',
        'максимум', 'максимальн', 'max', 'наибольш',
        'минимум', 'минимальн', 'min', 'наименьш',
        'количество', 'count', 'сколько всего', 'сколько записей',
        'отсортируй', 'сортировка', 'sort', 'упорядочь',
        'закрепи', 'заморозь', 'freeze',
        'диаграмм', 'график', 'chart',
        'посчитай', 'вычисли', 'рассчитай',  # расчёт = sample (Python обработает)
    ]

    # Лимиты для режимов
    MAX_FULL_ROWS = 500  # Максимум строк для full mode
    MAX_SAMPLE_ROWS = 10  # Строк для sample mode
    SMALL_TABLE_THRESHOLD = 50  # Таблицы меньше этого - всегда full

    @staticmethod
    def determine_data_mode(query: str, row_count: int) -> str:
        """
        Определяет режим отправки данных: 'full' или 'sample'.

        Returns:
            'full' - отправлять все данные (для поиска, выделения, процентов)
            'sample' - отправлять только примеры (для агрегаций, формул)
        """
        query_lower = query.lower()

        # Маленькие таблицы - всегда full (дёшево)
        if row_count <= SimpleGPTProcessor.SMALL_TABLE_THRESHOLD:
            return "full"

        # Проверяем ключевые слова для full режима
        for keyword in SimpleGPTProcessor.FULL_DATA_KEYWORDS:
            if keyword in query_lower:
                return "full"

        # Проверяем ключевые слова для sample режима
        for keyword in SimpleGPTProcessor.SAMPLE_KEYWORDS:
            if keyword in query_lower:
                return "sample"

        # По умолчанию для средних таблиц - full, для больших - sample
        if row_count <= 200:
            return "full"
        else:
            return "sample"
    ...
    @staticmethod
    def format_ascii_table(df: pd.DataFrame, column_names: List[str], mode: str, max_rows: int = None) -> str:
        """
        Форматирует данные в ASCII-таблицу как в Google Sheets.

        Args:
            df: DataFrame с данными
            column_names: Названия колонок
            mode: 'full' или 'sample'
            max_rows: Максимум строк (если None, используются дефолты)

        Returns:
            ASCII-таблица с координатами (A1, B1, etc.)
        """
        if max_rows is None:
            max_rows = SimpleGPTProcessor.MAX_FULL_ROWS if mode == 'full' else SimpleGPTProcessor.MAX_SAMPLE_ROWS

        # Ограничиваем количество строк
        total_rows = len(df)
        if total_rows > max_rows:
            if mode == 'sample':
                # Sample: первые N строк + последняя
                df_display = pd.concat([df.head(max_rows - 1), df.tail(1)])
                truncated = True
            else:
                # Full: первые max_rows строк
        ...
        # Генерируем буквы колонок (A, B, C, ..., Z, AA, AB, ...)
        def col_letter(n):
            result = ""
            while n >= 0:
        ...
        col_letters = [col_letter(i) for i in range(len(column_names))]

        # Формируем заголовок
        lines = []
        header = "    |" + "|".join(f" {col_letters[i]:^10} " for i in range(min(len(column_names), 10)))
        lines.append(header)
        lines.append("-" * len(header))

        # Строка 1 = заголовки колонок
        row_str = "  1 |"
        for i, col_name in enumerate(column_names[:10]):  # Макс 10 колонок
            val = str(col_name)[:10]
            row_str += f" {val:^10} |"
        ...
        # Данные
        for idx, (df_idx, row) in enumerate(df_display.iterrows()):
            row_num = df_idx + 2  # +2 потому что строка 1 = заголовки, индексы с 0
            row_str = f"{row_num:3} |"
            for i, val in enumerate(row[:10]):  # Макс 10 колонок
                val_str = str(val) if pd.notna(val) else ""
                val_str = val_str[:10]  # Обрезаем длинные значения
                row_str += f" {val_str:^10} |"
            lines.append(row_str)

            # Добавляем ... если данные обрезаны и это предпоследняя строка sample
            if truncated and mode == 'sample' and idx == max_rows - 2:
                lines.append("... |" + "|".join(f" {'...':^10} " for _ in range(min(len(column_names), 10))))

        # Информация о таблице
        if truncated:
            if mode == 'sample':
                lines.append(f"\n[Показано {len(df_display)} из {total_rows} строк. Режим: SAMPLE]")
            else:
                lines.append(f"\n[Показано {len(df_display)} из {total_rows} строк. Режим: FULL]")
        else:
            lines.append(f"\n[Всего: {total_rows} строк. Режим: {'FULL' if mode == 'full' else 'SAMPLE'}]")
...
КРИТИЧНО - ОБРАБОТКА ЧИСЛОВЫХ КОЛОНОК:
- ВСЕ колонки с данными сначала конвертируй в числа: pd.to_numeric(df[col], errors='coerce')
- После конвертации заполни NaN нулями: .fillna(0)
- ШАБЛОН для числовых колонок:
  for col in numeric_cols:
      df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
- При расчёте средних/сумм используй: df[cols].mean(axis=1) или df[cols].sum(axis=1)
- НИКОГДА не делай .astype(int) без предварительной обработки NaN!

КРИТИЧНО - НЕ ВЫВОДИ nan/NaN В ОТВЕТЕ:
- НИКОГДА не показывай "nan руб." или "nan%" пользователю!
- При расчёте процентов: если база = 0, пиши "новый" или "появился"
- При расчёте изменений: если данных нет, пиши "нет данных"
- ПРИМЕР расчёта процента изменения:
  pct = ((new - old) / old * 100) if old != 0 else (100 if new > 0 else 0)
- Для форматирования: f"{val:.1f}%" if not pd.isna(val) else "новый"
- ПРОВЕРЯЙ результаты перед выводом: str(val).replace('nan', 'н/д')

КРИТИЧНО - ИСКЛЮЧАЙ ИТОГОВЫЕ СТРОКИ:
- ПЕРЕД расчётами ВСЕГДА фильтруй строки с итогами:
  df_clean = df[~df[col].str.lower().str.contains('итог|итого|всего|total|sum|сумма', na=False)]
- Итоговые строки НЕ должны участвовать в группировке, расчёте процентов и т.д.
- Если % от итога, считай: (значение / df_clean[col].sum()) * 100
- НЕ ВКЛЮЧАЙ строки "Итог" как отдельную категорию!
...
- title должен отражать запрос"""
    ...
    async def _gpt_smart_action(self, query: str, column_names: List[str], df: pd.DataFrame, history: List[Dict[str, Any]] = None, custom_context: Optional[str] = None, reference_df: pd.DataFrame = None, reference_sheet_name: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            - None если нужен Python код для сложного анализа
            - Dict с готовым ответом для frontend
        """
        import json
...
"""
        ...
        # Формируем ASCII-таблицу с координатами (как в Google Sheets)
        ascii_table = self.format_ascii_table(df, column_names, data_mode)

        # Дополнительно готовим JSON для highlight (нужны точные номера строк)
        data_rows = []
        rows_for_json = df.head(self.MAX_FULL_ROWS) if data_mode == 'full' else df.head(self.MAX_SAMPLE_ROWS)
        for idx, row in rows_for_json.iterrows():
            row_data = {f"col_{i}_{column_names[i]}": str(v) for i, v in enumerate(row) if i < len(column_names)}
            row_data["_row_number"] = idx + 2  # +2 for 1-indexing and header
            data_rows.append(row_data)

        # Column info с типами
        col_info = []
        for idx, col in enumerate(column_names):
            if idx < len(df.columns):
                col_data = df.iloc[:, idx]
                try:
                    ...
                    if numeric.notna().sum() / len(numeric) > 0.5:
                        col_info.append(f"{idx}. '{col}' (числовая, min={numeric.min():.1f}, max={numeric.max():.1f}, sum={numeric.sum():.1f})")
                    else:
                        samples = col_data.dropna().head(3).tolist()
                        unique_count = col_data.nunique()
                        col_info.append(f"{idx}. '{col}' (текст, {unique_count} уник.): {samples}")
                except:
                    col_info.append(f"{idx}. '{col}'")

        # Для full режима показываем JSON только при highlight запросах
        is_highlight_request = any(kw in query.lower() for kw in ['выдели', 'выделить', 'подсвети', 'highlight'])
        data_json_section = ""
        if data_mode == 'full' and is_highlight_request:
        ...
        if reference_df is not None and reference_sheet_name:
            ...
            # v10.1.3: Show more rows for VLOOKUP - up to 50 rows to cover most use cases
            ref_rows_sample = reference_df.head(50).to_string()
...
⚠️ ОБЯЗАТЕЛЬНЫЙ ФОРМАТ для VLOOKUP:
{{"action_type": "vlookup", "key_column": "Артикул", "value_column": "НАЗВАНИЕ_КОЛОНКИ_ДЛЯ_ПОДТЯГИВАНИЯ", "summary": "..."}}

ПРИМЕР: Если просят "подтяни данные за октябрь":
{{"action_type": "vlookup", "key_column": "Артикул", "value_column": "Октябрь", "summary": "Подтягиваю октябрь по артикулам."}}

🔴 ОБЯЗАТЕЛЬНО указывай value_column - название колонки из справочного листа!
"""

        # v10.2.4: Detect forecast query and add CRITICAL instruction
        forecast_critical = ""
        if any(kw in query.lower() for kw in ['прогноз', 'спрогнозир', 'forecast', 'предсказ']):
            forecast_critical = """
🚨🚨🚨 КРИТИЧЕСКАЯ ИНСТРУКЦИЯ ДЛЯ ПРОГНОЗА 🚨🚨🚨

ЭТО ЗАПРОС НА ПРОГНОЗ! ВЫПОЛНИ СТРОГО ПО ПРАВИЛАМ:

1. ФОРМАТ ОТВЕТА (ОБЯЗАТЕЛЬНО!):
{
  "action_type": "write_data",
  "merge_by_key": "Артикул",
  "write_headers": ["Артикул", "Прогноз Январь"],
  "write_data": [["СХ_андер_54", 4696], ["СХ_анимезхс_54", 2520], ...],
  "summary": "📊 Добавил колонку прогноза. Метод: [ЧЕСТНО НАПИШИ КАКОЙ]"
}

2. ЗАПРЕЩЕНО:
❌ structured_data - ЗАПРЕЩЕНО (создаст отдельный лист!)
❌ display_mode - ЗАПРЕЩЕНО
❌ action_type: null - ЗАПРЕЩЕНО

3. РАСЧЁТ ТРЕНДА (НЕ среднее!):
- Для каждого артикула: тренд = последнее_значение / первое_значение
- Прогноз = последнее_значение × (1 + месячный_рост)
- Если данные стабильны (колебания <10%) - ТОГДА можно среднее

4. ЧЕСТНОСТЬ:
- Если использовал среднее - пиши "простое среднее"
- Если использовал тренд - пиши "экстраполяция тренда +X%"

🚨🚨🚨 КОНЕЦ КРИТИЧЕСКОЙ ИНСТРУКЦИИ 🚨🚨🚨

"""

        prompt = f"""{forecast_critical}{context_text}{history_text}{reference_sheet_section}Ты - ДУМАЮЩИЙ ЭКСПЕРТ по таблицам. Не исполнитель команд, а настоящий специалист.
...
ТАБЛИЦА (формат Google Sheets с координатами A1, B1...):
```
{ascii_table}
```{data_json_section}
...
==============================================================
[!!!] ИНТЕРПРЕТАЦИЯ ЗАПРОСОВ ПО ИМЕЮЩИМСЯ ДАННЫМ [!!!]
==============================================================
...
==============================================================
[!!!] ИСПОЛЬЗУЙ ТОЧНЫЕ НАЗВАНИЯ ИЗ ТАБЛИЦЫ [!!!]
==============================================================

КРИТИЧНО: Когда пишешь названия товаров, городов, менеджеров и т.д. -
копируй их ТОЧНО как в таблице!

❌ НЕПРАВИЛЬНО: "Веб: камера", "Веб камера", "веб-камера"
✅ ПРАВИЛЬНО: "Веб-камера" (точно как в данных)

❌ НЕПРАВИЛЬНО: "Санкт Петербург", "СПб", "С-Петербург"
✅ ПРАВИЛЬНО: "Санкт-Петербург" (точно как в данных)

НЕ ИЗМЕНЯЙ регистр, дефисы, пробелы - бери значения КАК ЕСТЬ!
...
ФОРМАТ ОТВЕТА С РАСЧЁТОМ (ПРОЗРАЧНОСТЬ!):
1. 📋 ДАННЫЕ: Укажи какие данные используешь (колонка, конкретные значения)
2. 🧮 МЕТОД: Покажи формулу/логику ("беру X, делю на Y, потому что...")
3. 📊 РЕЗУЛЬТАТ: Дай результат с объяснением что он значит
4. ⚠️ ОГОВОРКИ: Если есть ограничения или допущения - скажи о них

ПОЛЬЗОВАТЕЛЬ НЕ ДОЛЖЕН ПЕРЕСПРАШИВАТЬ "А КАК ТЫ ЭТО ПОСЧИТАЛ?"
Если после твоего ответа хочется спросить "откуда это?" - ответ ПЛОХОЙ!
...
==============================================================
[!!!] УМНОЕ ПРОГНОЗИРОВАНИЕ И АНАЛИЗ ДИНАМИКИ [!!!]
==============================================================

Когда просят ПРОГНОЗ или анализ ДИНАМИКИ - НЕ используй простое среднее!

ПРАВИЛЬНЫЙ ПОДХОД:
1. СНАЧАЛА определи тренд:
   - Данные растут? Падают? Стабильны?
   - Рассчитай: последнее значение vs первое, или месяц к месяцу

2. УЧИТЫВАЙ ТРЕНД в прогнозе:
   ❌ ПЛОХО: (сент+окт+нояб+дек)/4 = среднее (игнорирует рост!)
   ✅ ХОРОШО: Тренд +15%/мес → январь ≈ декабрь × 1.15

3. ПОКАЗЫВАЙ ХОД РАССУЖДЕНИЙ:
   "Смотрю динамику: сент→дек выросло с 2170 до 3913 (+80% за 3 мес, ~+20%/мес)
   При сохранении тренда: январь ≈ 3913 × 1.2 = 4696
   Но январь часто слабее декабря, поэтому консервативно: ~4200"

4. УПОМИНАЙ ОГРАНИЧЕНИЯ:
   - "Это простой прогноз без учёта сезонности"
   - "Для точности нужны данные за прошлый год"
   - "Тренд может не сохраниться"

МЕТОДЫ ПРОГНОЗИРОВАНИЯ (от простого к сложному):
• Простое среднее - ТОЛЬКО если нет тренда (данные "ходят" вокруг среднего)
• Экстраполяция тренда - если есть рост/падение
• Взвешенное среднее - последние месяцы важнее
• Сезонный коэффициент - если есть данные за прошлый год

ПРИМЕР ПРОЗРАЧНОГО ОТВЕТА НА ПРОГНОЗ:
"📊 АНАЛИЗ ДИНАМИКИ для СХ_андер_54:
• Сентябрь: 2170
• Октябрь: 2245 (+3%)
• Ноябрь: 3282 (+46%)
• Декабрь: 3913 (+19%)

📈 ТРЕНД: Устойчивый рост, в среднем +23% в месяц

🎯 ПРОГНОЗ на январь:
• Метод тренда: 3913 × 1.23 = 4813
• Консервативно (январь слабее): 3913 × 1.10 = 4304
• Рекомендуемый диапазон: 4300-4800 шт

⚠️ ОГРАНИЧЕНИЯ: Прогноз без учёта сезонности. Январь после праздников может быть слабее."

⛔⛔⛔ КАТЕГОРИЧЕСКИЙ ЗАПРЕТ ДЛЯ ПРОГНОЗОВ ⛔⛔⛔

ЗАПРЕЩЕНО при запросе прогноза:
❌ НЕ ИСПОЛЬЗУЙ structured_data - это создаёт отдельный лист!
❌ НЕ ИСПОЛЬЗУЙ display_mode: "preview" - это создаёт отдельный лист!
❌ НЕ ИСПОЛЬЗУЙ простое среднее и не называй его "экстраполяцией тренда"!

ОБЯЗАТЕЛЬНО при запросе прогноза:
✅ ИСПОЛЬЗУЙ action_type: "write_data" с merge_by_key
✅ РЕАЛЬНО рассчитай тренд: (последнее / первое) для каждой строки
✅ В summary ЧЕСТНО напиши какой метод использовал

ПРОВЕРЬ СЕБЯ перед ответом:
1. Я использую "write_data", а НЕ "structured_data"?
2. Я действительно посчитал рост/падение для каждого артикула?
3. Если тренд +20%/месяц, прогноз = последнее_значение × 1.20?
4. Если я просто взял среднее - я написал "простое среднее", а НЕ "экстраполяция"?

ФОРМАТ ПРОГНОЗА (ТОЛЬКО ТАК!):
{{"action_type": "write_data", "merge_by_key": "Артикул", "write_headers": ["Артикул", "Прогноз Январь"], "write_data": [["СХ_андер_54", 4696], ["СХ_анимезхс_54", 2520], ...], "explanation": "Для каждого артикула рассчитал тренд (последний месяц / первый месяц) и экстраполировал на январь.", "summary": "📊 Добавил колонку прогноза в текущую таблицу. Метод: экстраполяция индивидуального тренда для каждого артикула."}}

❗ Если вернёшь structured_data - создастся отдельный лист и пользователь будет недоволен!
❗ Если соврёшь про метод расчёта - это будет видно по цифрам!

==============================================================
[!!!] ДОБАВЛЯЙ ВЫВОДЫ И РЕКОМЕНДАЦИИ [!!!]
==============================================================

После показа данных ОБЯЗАТЕЛЬНО добавь:

1. ГЛАВНЫЙ ВЫВОД (1-2 предложения):
   - Что является ключевой причиной/фактором?
   - Какой общий тренд видно?

2. ИНСАЙТ (что неочевидно):
   - Связи между данными
...
ПРИМЕР (анализ падения продаж):
"По менеджерам:
• Иванов: -387,000 руб. (-100%)
• Смирнов: -599,000 руб. (-70%)
...

📊 ВЫВОД: Главная причина падения - уход Иванова (100% падение) и проблемы у Смирнова (-70%).

💡 ИНСАЙТ: Иванов полностью прекратил продажи - возможно уволился или в отпуске.
Смирнов потерял клиентов в Санкт-Петербурге (там тоже -100%).

🎯 РЕКОМЕНДАЦИЯ: Проверить статус Иванова. Выяснить причину потери клиентов в СПб."

НЕ ПРОСТО ПОКАЗЫВАЙ ЦИФРЫ - ОБЪЯСНЯЙ ЧТО ОНИ ЗНАЧАТ!
...
⚠️⚠️⚠️ ОБЯЗАТЕЛЬНО ANALYSIS (Python) для этих запросов:
   - "прогноз" / "спрогнозируй" / "forecast" / "предсказать"
   - "сколько всего" / "итого" / "сумма по всем"
   - "процент" / "доля" / "%" (расчёт процентов)
   - "рост" / "падение" / "динамика" / "trend"
   - любой РАСЧЁТ по данным (не визуальное оформление)

   [\!] НЕ ГЕНЕРИРУЙ summary без расчёта! Python посчитает реальные значения!
   [\!] Для прогноза верни: {{"action_type": "analysis", "reason": "Прогноз требует Python расчёта с трендом"}}
...
Думай вслух в <thinking>, потом действуй в <response>."""
        ...
        try:
            ...
            # v11.1.2: OVERRIDE - highlight for search/filter queries should ALWAYS use Python
            ...
            # If GPT wants to chat/clarify with user
            if result.get("action_type") == "chat":
                logger.info(f"[SmartGPT] Chat response: {result.get('message')}")
                return {
                    "action_type": "chat",
                    "message": result.get("message", "Уточните ваш запрос"),
                    "success": True,
                    "result_type": "chat"
                }

            # Add summary to result for display
...
Нажмите кнопку ниже, чтобы заменить данные в таблице."""
    ...
    async def process(
        ...
        try:
            ...
            # If SmartGPT returned None, continue to Python code analysis
            logger.info(f"[SmartGPT] Analysis mode - using Python code")
...
⚠️⚠️⚠️ КРИТИЧЕСКИ ВАЖНО - CROSS-SHEET VLOOKUP ⚠️⚠️⚠️
Пользователь просит ПОДТЯНУТЬ данные из листа "{ref_name}".
Данные из этого листа УЖЕ ЗАГРУЖЕНЫ в переменную reference_df!

🔴 НЕ СПРАШИВАЙ пользователя где данные - они УЖЕ В reference_df!
🔴 НЕ ГОВОРИ "нет данных" - данные ЕСТЬ в reference_df!
🟢 ПРОСТО СДЕЛАЙ merge/join между df и reference_df!

ДОПОЛНИТЕЛЬНЫЙ СПРАВОЧНИК (reference_df) - лист "{ref_name}":
{ref_prompt}

ОБЯЗАТЕЛЬНЫЕ ДЕЙСТВИЯ для VLOOKUP (подтягивание данных между листами):
