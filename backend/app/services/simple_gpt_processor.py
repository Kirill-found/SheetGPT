...
📌 ПРОГНОЗ ДЛЯ КАЖДОЙ СТРОКИ = ДОБАВЬ КОЛОНКУ!
Если пользователь просит спрогнозировать для ВСЕХ артикулов/строк:
- НЕ создавай отдельную таблицу/лист!
- ДОБАВЬ колонку "Прогноз" к ТЕКУЩЕЙ таблице справа
- Используй action_type: "write_data" с merge_by_key (как VLOOKUP)

Формат:
{{"action_type": "write_data", "merge_by_key": "Артикул", "write_headers": ["Артикул", "Прогноз Январь"], "write_data": [["СХ_андер_54", 4500], ["СХ_анимезхс_54", 2100], ...], "summary": "📊 Добавил колонку 'Прогноз Январь'. Метод: экстраполяция тренда (+X% в месяц)."}}

Так данные останутся в ОДНОЙ таблице и пользователю не нужно переключаться!
