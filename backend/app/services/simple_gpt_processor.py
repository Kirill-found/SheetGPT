...
- title должен отражать запрос"""
    ...
    async def _gpt_smart_action(self, query: str, column_names: List[str], df: pd.DataFrame, history: List[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        ...
        data_rows = []
        for idx, row in df.head(50).iterrows():  # First 50 rows with indices
            ...
            if idx < len(df.columns):
                ...
                try:
                    ...
                    if numeric.notna().sum() / len(numeric) > 0.5:
                        col_info.append(f"{idx}. '{col}' (числовая, min={numeric.min():.1f}, max={numeric.max():.1f})")
                    else:
                        samples = col_data.dropna().head(3).tolist()
                        col_info.append(f"{idx}. '{col}' (текст): {samples}")
                except:
                    col_info.append(f"{idx}. '{col}'")

        prompt = f"""{history_text}Ты умный ассистент для работы с таблицами Google Sheets.
...
ДАННЫЕ (первые 50 строк с номерами):
{json.dumps(data_rows[:20], ensure_ascii=False, indent=2)}

ТВОЯ ЗАДАЧА:
Проанализируй запрос и данные. Верни JSON с готовым ответом для выполнения действия.

ФОРМАТ ОТВЕТА - выбери ОДИН подходящий тип:

1. Для ВЫДЕЛЕНИЯ СТРОК (highlight):
{{"action_type": "highlight", "highlight_rows": [номера строк], "highlight_color": "#цвет", "summary": "описание"}}

2. Для СОРТИРОВКИ:
{{"action_type": "sort", "sort_column_index": индекс, "sort_order": "asc"|"desc", "summary": "описание"}}

3. Для ЦВЕТОВОЙ ШКАЛЫ (градиент по значениям):
{{"action_type": "color_scale", "rule": {{"column_index": индекс, "column_name": "имя", "min_color": {{"red":0-1,"green":0-1,"blue":0-1}}, "mid_color": {{...}}, "max_color": {{...}}, "row_count": число}}, "summary": "описание"}}

4. Для УСЛОВНОГО ФОРМАТИРОВАНИЯ (по условию):
{{"action_type": "conditional_format", "rule": {{"column_index": индекс, "condition_type": "NUMBER_GREATER"|"NUMBER_LESS"|"TEXT_CONTAINS"|"BLANK", "condition_value": значение, "format_color": {{"red":0-1,"green":0-1,"blue":0-1}}}}, "summary": "описание"}}

5. Для ДИАГРАММЫ:
{{"action_type": "chart", "chart_spec": {{"chart_type": "COLUMN"|"LINE"|"PIE"|"BAR", "title": "заголовок", "x_column_index": индекс, "y_column_indices": [индексы], "row_count": число}}, "summary": "описание"}}

6. Для ЗАКРЕПЛЕНИЯ:
{{"action_type": "freeze", "freeze_rows": число, "freeze_columns": число, "summary": "описание"}}

7. Для СВОДНОЙ ТАБЛИЦЫ / ГРУППИРОВКИ (много строк данных):
{{"action_type": "pivot_table", "pivot_data": {{"headers": ["Группа", "Значение"], "rows": [["Группа1", 100], ["Группа2", 200]]}}, "summary": "описание"}}

8. Для ЗАПИСИ ЗНАЧЕНИЯ В ЯЧЕЙКУ (одно значение в конкретную ячейку):
{{"action_type": "write_value", "target_cell": "B12", "value": 55790, "summary": "описание"}}

9. Для АНАЛИЗА/ВЫЧИСЛЕНИЙ (когда нужен Python код):
{{"action_type": "analysis", "reason": "почему нужен анализ"}}

ВАЖНО:
- ВОПРОСЫ требуют ОТВЕТА, не действия! "какой?", "сколько?", "кто?", "что?", "найди" → action_type="analysis"
- Только КОМАНДЫ выполняют действия: "выдели", "отсортируй", "покрась", "закрепи", "построй", "сгруппируй", "вставь", "запиши"
- "вставь ЭТО в ячейку X", "запиши результат в Y" → write_value! Найди значение из ИСТОРИИ и запиши в указанную ячейку
- "вставь в таблицу", "создай сводную" (без указания ячейки) → pivot_table
- Если пользователь говорит "его", "её", "это", "результат" - найди КОНКРЕТНОЕ ЗНАЧЕНИЕ в ИСТОРИИ диалога
...
КРИТИЧЕСКИ ВАЖНО - ТРАНСФОРМАЦИЯ ДАННЫХ:
- "удали дубли/дубликаты", "удали пустые строки", "очисти данные" → analysis (требует Python для изменения данных!)
- "разбей ФИО", "раздели по ячейкам", "text to columns" → analysis (требует Python для трансформации!)
- НЕЛЬЗЯ использовать highlight для удаления или разбиения - это ИЗМЕНЕНИЕ ДАННЫХ, не визуализация!

ПРИМЕРЫ:
- "какой товар самый дорогой?" → analysis (ВОПРОС, нужен текстовый ответ)
- "выдели самый дорогой товар" → highlight (КОМАНДА, нужно действие)
- "сколько всего продаж?" → analysis (ВОПРОС)
- "покрась продажи градиентом" → color_scale (КОМАНДА)
- "вставь это в ячейку B12" после "средняя цена 55790" → write_value с target_cell="B12", value=55790
- "запиши результат в C5" → write_value (найди число из истории)
- "вставь в таблицу" после анализа сумм → pivot_table с этими суммами из истории
- "сгруппируй по товарам" → pivot_table
- "удали дубли" → analysis (ИЗМЕНЕНИЕ ДАННЫХ!)
- "удали пустые строки" → analysis (ИЗМЕНЕНИЕ ДАННЫХ!)
- "разбей ФИО по ячейкам" → analysis (ТРАНСФОРМАЦИЯ ДАННЫХ!)
