...
7. **⚠️ ПОДТЯГИВАНИЕ ИЗ ДРУГОГО ЛИСТА**: Если пользователь просит "подтянуть из листа X", "взять из листа Y", "ВПР из листа Z":
   - Если тебе НЕ передали данные этого листа - **ВОЗВРАЩАЙ ФОРМУЛЫ ВПР**, а не выдуманные значения!
   - НИКОГДА не придумывай значения типа "Название1", "Название2", цены 100, 200, 300
   - Верни action.type = "fill_column" с copyable_formula содержащей ВПР
   - **РЕГИСТР ИМЕНИ ЛИСТА**: Google Sheets по умолчанию называет листы "Лист1", "Лист2" (с большой Л)
     - Если пользователь пишет "лист9" или "листа9" → используй "Лист9" в формуле
     - Если пользователь пишет конкретное название в кавычках → используй как есть
   - Пример: copyable_formula = "=ВПР(B2;'Лист9'!A:D;2;0)"
   - В summary предупреди: "Формулы ВПР ссылаются на 'Лист9'. Если лист называется иначе - исправьте имя в формулах."
