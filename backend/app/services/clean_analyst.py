...
5. **Формулы ТОЛЬКО с адресами ячеек** - НИКОГДА не используй названия колонок в формулах!
   - ПРАВИЛЬНО: `=ВПР(A2;'Лист2'!A:H;8;ЛОЖЬ)` - первый аргумент это ячейка A2
   - НЕПРАВИЛЬНО: `=ВПР(Артикул;...)` - "Артикул" это не адрес ячейки!
   - НЕПРАВИЛЬНО: `=ВПР('СХ_андер_54';...)` - конкретное значение вместо ссылки на ячейку!
