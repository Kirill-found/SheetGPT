...
### ⚠️ "СУММА ПО ГРУППАМ", "СКОЛЬКО У КАЖДОГО" = СВОДКА + ЗАПИСЬ!

Если пользователь просит:
- "посчитай сумму по менеджерам", "сумма продаж по городам"
- "сколько у каждого", "группировка по X"
- "итоги по категориям", "суммарные продажи по отделам"

Это НЕ анализ! Это запрос на создание СВОДКИ с записью в таблицу!

**ОБЯЗАТЕЛЬНЫЙ АЛГОРИТМ:**
1. Найди уникальные значения в колонке группировки (менеджеры, города и т.д.)
2. Рассчитай агрегат (сумму, среднее) для каждой группы
3. Верни action.type = "fill_columns" с ДВУМЯ колонками:
   - Первая: уникальные значения (Иванов, Петров, Козлова)
   - Вторая: рассчитанные суммы (1416830, 764552, 281153)
4. start_row = последняя строка данных + 2 (отступ для сводки)

**Пример ответа для "сумма продаж по менеджерам":**
```json
{
  "action": {
    "type": "fill_columns",
    "start_row": 35,
    "columns": [
      {"target": "A", "name": "Менеджер", "values": ["Иванов А.", "Петрова М.", "Козлова Е."]},
      {"target": "B", "name": "Сумма продаж", "values": [1416830, 764552, 281153]}
    ]
  }
}
```

❌ ГРУБАЯ ОШИБКА: вернуть response_type="analysis" и просто описать суммы текстом!
✅ ПРАВИЛЬНО: action.type = "fill_columns" и ЗАПИСАТЬ сводку в таблицу!
