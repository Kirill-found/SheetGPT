...
class FunctionRegistry:
    ...
    def get_function_definitions(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {
                "name": "sort_data",
                "description": "Сортировка данных по одной или нескольким колонкам. ОБЯЗАТЕЛЬНО используй для 'отсортируй', 'упорядочь', 'покажи от новых к старым', 'от старых к новым', 'сначала новые', 'сначала старые', 'по дате', 'по возрастанию', 'по убыванию', 'от меньшего к большему', 'от большего к меньшему'. Примеры: 'покажи заказы от новых к старым' → columns=['Дата'], ascending=False; 'от старых к новым' → columns=['Дата'], ascending=True; 'отсортируй по сумме' → columns=['Сумма'], ascending=True; 'сначала самые дорогие' → columns=['Сумма'], ascending=False",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "columns": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "Список колонок для сортировки. Для временных запросов используй колонку с датой (обычно 'Дата', 'Date' или похожую)"
                        },
                        "ascending": {
                            "type": "boolean",
                            "description": "True для сортировки по возрастанию (от старых к новым, от меньших к большим), False для убывания (от новых к старым, от больших к меньшим, сначала самые дорогие). ВАЖНО: 'от новых к старым' = False, 'от старых к новым' = True",
                            "default": True
                        }
                    },
                    "required": ["columns"]
