...
# Split ALL rows including the first one
...
methodology = f"Разделение данных по разделителю ('{{delimiter}}'). Первая строка использована как заголовки столбцов. Обнаружено {{len(result)}} строк данных"
```
