...
Запрос: "Какие продукты продал Иванов" или "какие товары у менеджера"
Код:
```python
result = df[df['Менеджер'] == 'Иванов']['Продукт'].unique().tolist()
```

Запрос: "Какие города есть в таблице" или "список городов"
Код:
```python
result = df['Город'].dropna().unique().tolist()
```

Запрос: "Покажи все заказы Иванова" или "что купил Иванов"
Код:
```python
result = df[df['Менеджер'] == 'Иванов']
```

ВАЖНО:
- Если запрос "какие/какой/что" - возвращай СПИСОК конкретных значений, НЕ количество!
- Если запрос "сколько" - возвращай число (count)
...
"""
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                model="gpt-4o",  # Best quality for complex code generation
                messages=[
